...
/ * make sure it is safe to use LARGE_INTEGER and LONGLONG types 
	...
	#include <windows.h>
bool GetPerformanceCounter(LARGE_INTEGER *performanceCount);

inline bool GetPerformanceCounter(LARGE_INTEGER *performanceCount)
{
	return QueryPerformanceCounter((LARGE_INTEGER*) &performanceCount);
}
